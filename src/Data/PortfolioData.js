export const PortfolioData = [
  {
    id: 1,
    name: "Portfolio Personnel",
    languages: ["React", "SCSS", "HTML", "CSS"],
    languagesIcons: [
      "fab fa-react",
      "fab fa-sass",
      "fab fa-html5",
      "fab fa-css3-alt",
    ],
    source: "https://github.com/AdhuleJean/portfolio",
    info: "Un site web personnel moderne permettant de présenter mes compétences, projets et expériences professionnelles.",
    picture: process.env.PUBLIC_URL + "/Media/total_img.jpg",
  },
  {
    id: 2,
    name: "Application de Gestion de Tâches",
    languages: ["JavaScript", "Node.js", "MongoDB", "Bootstrap"],
    languagesIcons: [
      "fab fa-js",
      "fab fa-node",
      "fas fa-database",
      "fab fa-bootstrap",
    ],
    source: "https://github.com/AdhuleJean/todo-app",
    info: "Application web full-stack permettant la gestion des tâches avec authentification utilisateur et base de données MongoDB.",
    picture: process.env.PUBLIC_URL + "/Media/to_do_app.png",
  },
  {
    id: 3,
    name: "Détecteur d'Alcool avec Arduino",
    languages: ["C++", "Python"],
    languagesIcons: ["fab fa-python", "fas fa-microchip"],
    source: "https://github.com/AdhuleJean/alcohol-detector",
    info: "Projet IoT intégrant Arduino et un capteur MQ3, avec affichage des résultats sur une interface Python (PySide6).",
    picture: process.env.PUBLIC_URL + "/Media/alcohol_detector.png",
  },
  {
    id: 4,
    name: "Application Mobile de Reconnaissance Faciale",
    languages: ["Dart", "Flutter", "Python"],
    languagesIcons: ["fab fa-android", "fab fa-python", "fas fa-robot"],
    source: "https://github.com/AdhuleJean/face-recognition",
    info: "Application mobile Flutter connectée à une API Python/InsightFace pour la reconnaissance faciale et la sécurité.",
    picture: process.env.PUBLIC_URL + "/Media/face_recognition.png",
  },
  {
    id: 5,
    name: "Blog Technique avec Symfony",
    languages: ["PHP", "Symfony", "MySQL", "jQuery"],
    languagesIcons: [
      "fab fa-php",
      "fas fa-database",
      "fab fa-symfony",
      "fas fa-code",
    ],
    source: "https://github.com/AdhuleJean/blog-symfony",
    info: "Un blog technique développé avec Symfony, gestion d’articles, commentaires et tableau d’administration.",
    picture: process.env.PUBLIC_URL + "/Media/blog_symfony.png",
  },
  {
    id: 6,
    name: "Tableau de Bord Analytique",
    languages: ["JavaScript", "React", "Bootstrap", "CSS"],
    languagesIcons: [
      "fab fa-js",
      "fab fa-react",
      "fab fa-bootstrap",
      "fab fa-css3-alt",
    ],
    source: "https://github.com/AdhuleJean/dashboard-analytics",
    info: "Dashboard interactif avec graphiques, filtres et intégration d'API REST pour l’analyse des données.",
    picture: process.env.PUBLIC_URL + "/Media/dashboard.png",
  },
  {
    id: 7,
    name: "Site de E-commerce",
    languages: ["PHP", "MySQL", "Bootstrap", "jQuery"],
    languagesIcons: [
      "fab fa-php",
      "fas fa-database",
      "fab fa-bootstrap",
      "fas fa-code",
    ],
    source: "https://github.com/AdhuleJean/ecommerce-site",
    info: "Site e-commerce complet avec panier d’achats, gestion des utilisateurs et paiement en ligne simulé.",
    picture: process.env.PUBLIC_URL + "/Media/e_commerce.png",
  },
  {
    id: 8,
    name: "Simulateur de Capteurs IoT",
    languages: ["Python", "React", "Node.js"],
    languagesIcons: ["fab fa-python", "fab fa-react", "fab fa-node"],
    source: "https://github.com/AdhuleJean/iot-simulator",
    info: "Application simulant des capteurs IoT et affichant les données en temps réel sur une interface React.",
    picture: process.env.PUBLIC_URL + "/Media/iot.png",
  },
    {
    id: 9,
    name: "Portfolio Personnel",
    languages: ["React", "SCSS", "HTML", "CSS"],
    languagesIcons: [
      "fab fa-react",
      "fab fa-sass",
      "fab fa-html5",
      "fab fa-css3-alt",
    ],
    source: "https://github.com/AdhuleJean/portfolio",
    info: "Un site web personnel moderne permettant de présenter mes compétences, projets et expériences professionnelles.",
    picture: process.env.PUBLIC_URL + "/Media/total_img.jpg",
  },
  {
    id: 10,
    name: "Application de Gestion de Tâches",
    languages: ["JavaScript", "Node.js", "MongoDB", "Bootstrap"],
    languagesIcons: [
      "fab fa-js",
      "fab fa-node",
      "fas fa-database",
      "fab fa-bootstrap",
    ],
    source: "https://github.com/AdhuleJean/todo-app",
    info: "Application web full-stack permettant la gestion des tâches avec authentification utilisateur et base de données MongoDB.",
    picture: process.env.PUBLIC_URL + "/Media/to_do_app.png",
  },
  {
    id: 11,
    name: "Détecteur d'Alcool avec Arduino",
    languages: ["C++", "Python"],
    languagesIcons: ["fab fa-python", "fas fa-microchip"],
    source: "https://github.com/AdhuleJean/alcohol-detector",
    info: "Projet IoT intégrant Arduino et un capteur MQ3, avec affichage des résultats sur une interface Python (PySide6).",
    picture: process.env.PUBLIC_URL + "/Media/alcohol_detector.png",
  },
  {
    id: 12,
    name: "Application Mobile de Reconnaissance Faciale",
    languages: ["Dart", "Flutter", "Python"],
    languagesIcons: ["fab fa-android", "fab fa-python", "fas fa-robot"],
    source: "https://github.com/AdhuleJean/face-recognition",
    info: "Application mobile Flutter connectée à une API Python/InsightFace pour la reconnaissance faciale et la sécurité.",
    picture: process.env.PUBLIC_URL + "/Media/face_recognition.png",
  },
  {
    id: 13,
    name: "Blog Technique avec Symfony",
    languages: ["PHP", "Symfony", "MySQL", "jQuery"],
    languagesIcons: [
      "fab fa-php",
      "fas fa-database",
      "fab fa-symfony",
      "fas fa-code",
    ],
    source: "https://github.com/AdhuleJean/blog-symfony",
    info: "Un blog technique développé avec Symfony, gestion d’articles, commentaires et tableau d’administration.",
    picture: process.env.PUBLIC_URL + "/Media/blog_symfony.png",
  },
  {
    id: 14,
    name: "Tableau de Bord Analytique",
    languages: ["JavaScript", "React", "Bootstrap", "CSS"],
    languagesIcons: [
      "fab fa-js",
      "fab fa-react",
      "fab fa-bootstrap",
      "fab fa-css3-alt",
    ],
    source: "https://github.com/AdhuleJean/dashboard-analytics",
    info: "Dashboard interactif avec graphiques, filtres et intégration d'API REST pour l’analyse des données.",
    picture: process.env.PUBLIC_URL + "/Media/dashboard.png",
  },
  {
    id: 15,
    name: "Site de E-commerce",
    languages: ["PHP", "MySQL", "Bootstrap", "jQuery"],
    languagesIcons: [
      "fab fa-php",
      "fas fa-database",
      "fab fa-bootstrap",
      "fas fa-code",
    ],
    source: "https://github.com/AdhuleJean/ecommerce-site",
    info: "Site e-commerce complet avec panier d’achats, gestion des utilisateurs et paiement en ligne simulé.",
    picture: process.env.PUBLIC_URL + "/Media/e_commerce.png",
  },
  {
    id: 16,
    name: "Simulateur de Capteurs IoT",
    languages: ["Python", "React", "Node.js"],
    languagesIcons: ["fab fa-python", "fab fa-react", "fab fa-node"],
    source: "https://github.com/AdhuleJean/iot-simulator",
    info: "Application simulant des capteurs IoT et affichant les données en temps réel sur une interface React.",
    picture: process.env.PUBLIC_URL + "/Media/iot.png",
  },
   {
    id: 9,
    name: "Gestion d'École avec Spring Boot",
    languages: ["Java", "Spring Boot", "MySQL"],
    languagesIcons: ["fab fa-java", "fas fa-database", "fas fa-server"],
    source: "https://github.com/AdhuleJean/school-management",
    info: "Application web Java Spring Boot pour la gestion d’une école (étudiants, enseignants, cours, notes) avec interface web et API REST.",
    picture: "https://cdn.pixabay.com/photo/2020/08/17/08/31/school-5496582_1280.jpg",
  },
  {
    id: 10,
    name: "Application Desktop de Gestion Financière",
    languages: ["Java", "JavaFX", "SQLite"],
    languagesIcons: ["fab fa-java", "fas fa-chart-line", "fas fa-database"],
    source: "https://github.com/AdhuleJean/finance-manager",
    info: "Application JavaFX moderne pour gérer les revenus et dépenses, avec statistiques et interface utilisateur intuitive.",
    picture: "https://cdn.pixabay.com/photo/2016/02/19/11/30/money-1202398_1280.jpg",
  },
  {
    id: 11,
    name: "API REST de Gestion RH",
    languages: ["Java", "Spring Boot", "Hibernate"],
    languagesIcons: ["fab fa-java", "fas fa-server", "fas fa-database"],
    source: "https://github.com/AdhuleJean/hr-api",
    info: "API Java Spring Boot pour la gestion des employés, des départements et des congés, avec persistance via Hibernate.",
    picture: "https://cdn.pixabay.com/photo/2019/06/14/13/08/api-4279513_1280.jpg",
  },
  {
    id: 12,
    name: "Application Android de Gestion de Stock",
    languages: ["Java", "Android", "Firebase"],
    languagesIcons: ["fab fa-android", "fab fa-java", "fas fa-cloud"],
    source: "https://github.com/AdhuleJean/inventory-android",
    info: "Application Android native développée en Java pour la gestion de stock avec synchronisation Firebase en temps réel.",
    picture: "https://cdn.pixabay.com/photo/2018/03/01/09/35/mobile-phone-3190194_1280.jpg",
  },
];
